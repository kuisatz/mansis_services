 ...
 * @since 05.08.2018
 */
$app->get("/pkProjectVehicleModelsTradeDdList_infoprojectbuybacks/", function () use ($app ) { 
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('infoProjectBuybackBLL');
    ...
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkProjectVehicleModelsTradeDdList_infoprojectbuybacks" end point, X-Public variable not found');
   // $pk = $headerParams['X-Public'];
