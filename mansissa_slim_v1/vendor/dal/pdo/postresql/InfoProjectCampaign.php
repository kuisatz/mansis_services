 ...
 * @since 30.07.2018                         
 */ 
class InfoProjectCampaign extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN    
     * @ info_project_campaign tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ info_project_campaign tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN info_project_campaign ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ info_project_campaign tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO info_project_campaign(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('info_project_campaign_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ info_project_campaign tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * info_project_campaign tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için info_project_campaign tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için info_project_campaign tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ info_project_campaign tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ deal a eklenen garanti aracları dropdown ya da tree ye doldurmak için info_project_campaign tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException 
     */
    public function projectVehicleCampaignDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
            $errorInfo[0] = "99999";         
            $addSQL =null;
            ...
            $sql =  "    
                ...
                inner join info_project_campaign pvm on pvm.vehicles_endgroup_id = a.act_parent_id AND pvm.active =0 AND pvm.deleted =0  
    ...
    /** 
     * @author Okan CIRAN
     * @ deal aracların garanti tanımlarını grid formatında döndürür !! ana tablo  info_project_campaign 
     * @version v 1.0  20.08.2018
     * @param array | null $args 
     * @return array
     * @throws \PDOException  
     */  
    public function fillProjectCampaignGridx($params = array()) {
        try {
		    ...
		    INNER JOIN sys_campaign_vehicles bbm ON bbm.act_parent_id = a.campaign_vehicle_id AND bbm.show_it = 0  
		    INNER JOIN sys_campaigns c ON c.act_parent_id = bbm.campaign_id AND c.show_it =0 
    ...
    /** 
     * @author Okan CIRAN
     * @  deal aracların garanti tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo info_project_campaign
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillProjectCampaignGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSQL = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN
     * @ info_project_campaign tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ info_project_campaign tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
    ...
    public function insertAct($params = array()) {
        try {
            ...
            if ((isset($params['CampaignVehicleId']) && $params['CampaignVehicleId'] != "")) {
                $CampaignVehicleId = $params['CampaignVehicleId'];
            }  
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $kontrol = $this->haveRecords(
                        array(
                            ...
                            'campaign_vehicle_id' => $CampaignVehicleId,  
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES ( 
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM info_project_campaign_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
          //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
