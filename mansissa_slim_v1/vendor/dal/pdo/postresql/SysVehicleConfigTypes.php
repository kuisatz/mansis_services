 ...
 * @since 30.07.2018                      
 */ 
class SysVehicleConfigTypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN    
     * @ sys_vehicle_config_types tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    public function delete($params = array()) {
     try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                UPDATE sys_vehicle_config_types
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_config_types tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_vehicle_config_types ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_config_types tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_vehicle_config_types(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_vehicle_config_types_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_config_types tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_vehicle_config_types tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicle_config_types tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicle_config_types tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_vehicle_config_types tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ araç konfig tip tanımları dropdown ya da tree ye doldurmak için sys_vehicle_config_types tablosundan kayıtları döndürür !!
    ...
    public function  vehicleConfigTypesDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_vehicle_config_types sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
     ...
     * @throws \PDOException  
     */  
    public function fillVehicleConfigTypesGridx($params = array()) {
        try {
                    ...
                    if ($std['value'] != null) {
                        switch (trim($std['field'])) {
                            ...
                            case 'total_wheel':
                                ...
                                $sorguStr.=" AND a.total_wheel" . $sorguExpression . ' ';

                                break; 
                             case 'wheel_drive':
                                $sorguExpression = ' =  ' . $std['value'] . ' ';
                                $sorguStr.=" AND a.wheel_drive" . $sorguExpression . ' ';
    ...
    /** 
     * @author Okan CIRAN
     * @ araç konfig tip tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_vehicle_config_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillVehicleConfigTypesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_config_types tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_vehicle_config_types tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
    ...
    public function insertAct($params = array()) {
        try {
            ...
            $totalWheel = -1111;
            if ((isset($params['TotalWheel']) && $params['TotalWheel'] != "")) {
                $totalWheel = intval($params['TotalWheel']);
            } else {
                throw new \PDOException($errorInfo[0]);
            }
            $wheelDrive = -1111;
            if ((isset($params['WheelDrive']) && $params['WheelDrive'] != "")) {
                $wheelDrive = intval($params['WheelDrive']);
            } else {
                throw new \PDOException($errorInfo[0]);
            }
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM sys_vehicle_config_types_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
                    //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
