 ...
 * @since 29.07.2018   
 */ 
class SysCurrencyTypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_currency_types tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_currency_types tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_currency_types ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_currency_types tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_currency_types(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_currency_types_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_currency_types tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_currency_types tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_currency_types tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_currency_types tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_currency_types tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ doviz tanımları dropdown ya da tree ye doldurmak için sys_currency_types tablosundan kayıtları döndürür !!
    ...
    /** 
     * @author Okan CIRAN
     * @ doviz kod tanımları dropdown ya da tree ye doldurmak için sys_currency_types tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args
     * @return array 
     * @throws \PDOException 
     */
    public function  currencyCodeTypesDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');  
            $statement = $pdo->prepare("     
                SELECT                    
    ...
    /** 
     * @author Okan CIRAN
     * @ takvim  tipleri  tanımlarını grid formatında döndürür !! ana tablo  sys_currency_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException   
     */  
    public function fillCurrencyTypesGridx($params = array()) {
        try {
            ...
            if (isset($params['CurrencyID']) && $params['CurrencyID'] != "") {
                ...
                $addSql ="  a.act_parent_id  = " . intval($currencyID). "  AND  " ; 
            }  
                ...
                $sql = "
                    ...
                    LEFT JOIN sys_currency_types ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @ takvim  tipleri  tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_currency_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCurrencyTypesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
                   ...
                   SELECT COUNT(asdx.id) count FROM ( 
                        SELECT  
                          ...
                          /*  a.name_eng, */ 
                            COALESCE(NULLIF(sd16x.description, ''), sd16.description_eng) AS state_active, 
                            u.username AS op_user_name 
                        FROM sys_calendar_types a                    
                        INNER JOIN sys_language l ON l.id = a.language_id AND l.show_it =0
                        LEFT JOIN sys_language lx ON lx.id =" . intval($languageIdValue) . "    AND lx.show_it =0  
                        LEFT JOIN sys_calendar_types ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.show_it = 0 AND ax.language_id = lx.id
                        INNER JOIN info_users u ON u.id = a.op_user_id 
                        /*----*/   
                       /* INNER JOIN sys_specific_definitions sd15 ON sd15.main_group = 15 AND sd15.first_group= a.deleted AND sd15.deleted =0 AND sd15.active =0 AND sd15.language_parent_id =0 */
                        INNER JOIN sys_specific_definitions sd16 ON sd16.main_group = 16 AND sd16.first_group= a.active AND sd16.deleted = 0 AND sd16.active = 0 AND sd16.language_id =l.id
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_currency_types tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
    ...
    public function insertAct($params = array()) {
         ...
         */
          ...
          */
            ...
            $code = null;
            if ((isset($params['code']) && $params['Name'] != "")) {
                $code = $params['Name'];
            } else {
                throw new \PDOException($errorInfo[0]);
            }
            $currencyCode = null;
            if ((isset($params['CurrencyCode']) && $params['CurrencyCode'] != "")) {
                $currencyCode = $params['CurrencyCode'];
            } else {
                throw new \PDOException($errorInfo[0]);
            }
            $embraceCurCode = null;
            if ((isset($params['EmbraceCurCode']) && $params['EmbraceCurCode'] != "")) {
                $embraceCurCode = $params['EmbraceCurCode'];
            } else {
                throw new \PDOException($errorInfo[0]);
            }
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM sys_currency_types_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
                    //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_currency_types tablosuna aktif olan diller için ,tek bir kaydın tabloda olmayan diğer dillerdeki kayıtlarını oluşturur   !!
     * @version v 1.0  26.08.2018
    ...
    public function insertLanguageTemplate($params = array()) {
        try {
			...
			case l.id when 385 then c.id else (SELECT last_value FROM sys_currency_types_id_seq) end as act_parent_id,  
                        c.op_user_id
                    FROM sys_currency_types c
                    LEFT JOIN sys_language l ON l.deleted =0 AND l.active =0 
                    WHERE c.id = " . intval($params['id']) . "  
                    ) AS xy   
                    WHERE xy.language_id NOT IN 
                        (SELECT DISTINCT language_id 
                        FROM sys_currency_types cx 
                        WHERE 
