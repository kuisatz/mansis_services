 ...
 * @since 30.07.2018   
 */ 
class SysDemoQuotas extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_demo_quotas ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
        ...
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_demo_quotas_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_demo_quotas tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_demo_quotas tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_demo_quotas tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ supllier matrix ini grid formatında döndürür !! ana tablo  sys_demo_quotas 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException    
     */  
    public function fillDemoQuotasGridx($params = array()) {
        try {
                    ...
                    if ($std['value'] != null) {
                        switch (trim($std['field'])) {
                            case 'vehicle_group':
                                ...
                                $sorguStr.=" AND vgt.name" . $sorguExpression . ' ';
                              
                                break;
                             case 'vehicle_group_name':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\' ';
                                $sorguStr.=" AND vg.name" . $sorguExpression . ' ';
                              
                                break;  
                            case 'vehicle_type_name':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\' ';
                                $sorguStr.=" AND vgtp.name" . $sorguExpression . ' ';
                              
                                break;
                             case 'model_description':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\' ';
                                $sorguStr.=" AND vgt.model_description" . $sorguExpression . ' ';
                              
                                break;
                            case 'name_quota':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\'  ';
                                $sorguStr.=" AND COALESCE(NULLIF(sqx.name, ''), sq.name_eng)" . $sorguExpression . ' ';

                                break; 
                             case 'name_month':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\'  ';
                                $sorguStr.=" AND COALESCE(NULLIF(mx.name, ''), m.name_eng)" . $sorguExpression . ' ';
            ...
            $demoQuotaID=0 ;
            if (isset($params['DemoQuotaID']) && $params['DemoQuotaID'] != "") {
                $demoQuotaID = $params['DemoQuotaID'];
                $addSql .="  a.act_parent_id  = " . intval($demoQuotaID). "  AND  " ; 
            }  
            $vehicleTypeID =0 ;
            if (isset($params['VehicleTypeID']) && $params['VehicleTypeID'] != "") {
                $vehicleTypeID = $params['VehicleTypeID'];
                $addSql .="  vgtp.act_parent_id = " . intval($vehicleTypeID). "  AND  " ; 
            }  
           $monthId =0 ;
            if (isset($params['MonthId']) && $params['MonthId'] != "") {
             ...
             $quotaID =0 ;
            if (isset($params['QuotaID']) && $params['QuotaID'] != "") {
                $quotaID = $params['QuotaID'];
                $addSql .="  a.sis_quota_id  = " . intval($quotaID). "  AND  " ; 
            }  
			...
			vgt.name vehicle_group, 
                        vgt.model_description , 
			a.sis_quota_id,
			COALESCE(NULLIF(sqx.name, ''), sq.name_eng) AS name_quota, 
			a.month_id,
		    ...
		    left JOIN sys_vehicle_gt_models vgt ON vgt.act_parent_id = a.vehicle_gt_model_id AND vgt.show_it = 0  
		    left JOIN sys_vehicle_group_types vgtp ON vgtp.act_parent_id = a.vehicle_gt_model_id AND vgtp.show_it = 0  
		    left JOIN sys_vehicle_groups vg ON vg.act_parent_id = vgtp.vehicle_groups_id AND vg.show_it = 0  
 
		    INNER JOIN sys_sis_quotas sq ON sq.act_parent_id = a.sis_quota_id AND sq.show_it = 0 AND sq.language_id= l.id
		    LEFT JOIN sys_sis_quotas sqx ON (sqx.act_parent_id = sq.act_parent_id OR sqx.language_parent_id= sq.act_parent_id) AND sqx.show_it= 0 AND sqx.language_id =lx.id  
 
		    INNER JOIN sys_months m ON m.mvalue = a.month_id AND sq.show_it = 0 AND m.language_id= l.id AND m.parent_id = 9 
    ...
    /** 
     * @author Okan CIRAN
     * @ supplier matrix ini grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_demo_quotas 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillDemoQuotasGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
           ...
           $vehicleGroupID=0 ;
            ...
            if (isset($params['AccessoryOptionID']) && $params['AccessoryOptionID'] != "") {
                ...
                $addSql .="  asm.accessory_option_id  = " . intval($accessoryOptionID). "  AND  " ; 
            }  
            $accDeffID =0 ;
            if (isset($params['AccDeffID']) && $params['AccDeffID'] != "") {
                $accDeffID = $params['AccDeffID'];
                $addSql .="  asm.acc_deff_id  = " . intval($accDeffID). "  AND  " ; 
            }  
             $supplierID =0 ;
		    ...
		    INNER JOIN sys_months m ON m.mvalue = a.month_id AND sq.deleted = 0 AND m.active = 0 AND m.language_id= l.id AND m.parent_id = 9 
		    LEFT JOIN sys_months mx ON (mx.act_parent_id = m.act_parent_id OR mx.language_parent_id= m.act_parent_id) AND mx.show_it = 0 AND mx.language_id =lx.id  
     ...
     /**
     * @author Okan CIRAN
     * @ sys_demo_quotas tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_demo_quotas tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
