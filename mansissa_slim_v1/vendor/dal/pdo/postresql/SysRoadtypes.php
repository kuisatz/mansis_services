 ...
 * @since 30.07.2018     
 */ 
class SysRoadtypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN    
     * @ sys_roadtypes tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_roadtypes tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_roadtypes ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_roadtypes tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ sys_roadtypes tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_roadtypes tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_roadtypes tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_roadtypes tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ yol tipleri dropdown ya da tree ye doldurmak için sys_roadtypes tablosundan kayıtları döndürür !!
    ...
    public function  roadTypesDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_roadtypes sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
     ...
     * @throws \PDOException  
     */  
    public function fillRoadTypesGridx($params = array()) {
        try {
            ...
            if (isset($params['LanguageID']) && $params['LanguageID'] != "") {
                ...
                $sql = "
                    ...
                    LEFT JOIN sys_roadtypes ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted =0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @ yol tip  tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_roadtypes 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillRoadTypesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_roadtypes tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
    ...
    /**
     * @author Okan CIRAN
     * @ sys_roadtypes tablosuna aktif olan diller için ,tek bir kaydın tabloda olmayan diğer dillerdeki kayıtlarını oluşturur   !!
     * @version v 1.0  26.08.2018
    ...
    public function insertLanguageTemplate($params = array()) {
        try {
			...
			case l.id when 385 then c.id else (SELECT last_value FROM sys_roadtypes_id_seq) end as act_parent_id,  
                        c.op_user_id
                    FROM sys_roadtypes c
                    LEFT JOIN sys_language l ON l.deleted =0 AND l.active =0 
                    WHERE c.id = " . intval($params['id']) . "  
                    ) AS xy   
                    WHERE xy.language_id NOT IN 
                        (SELECT DISTINCT language_id 
                        FROM sys_roadtypes cx 
                        WHERE 
