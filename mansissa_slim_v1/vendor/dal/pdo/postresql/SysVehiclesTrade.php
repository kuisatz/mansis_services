 ...
 * @since 29.07.2018 
 */ 
class SysVehiclesTrade extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN   
     * @ sys_vehicles_trade tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicles_trade tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_vehicles_trade ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicles_trade tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_vehicles_trade(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_vehicles_trade_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicles_trade tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    public function haveRecords($params = array()) {
        try {
            ...
            WHERE  
                ...
                LOWER(REPLACE(description,' ','')) = LOWER(REPLACE('" . $params['description'] . "',' ','')) 
                " . $addSql . " 
                AND a.deleted =0      
                               ";
            $statement = $pdo->prepare($sql);
    ...
    /**
     * @author Okan CIRAN
     * sys_vehicles_trade tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicles_trade tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicles_trade tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_vehicles_trade tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ araç gruplarının buyback ve trade back tanımlarını dropdown ya da tree ye doldurmak için sys_vehicles_trade tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args     
     * @return array
     * @throws \PDOException 
     */
    public function vehiclesTbDefDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
    ...
    /** 
     * @author Okan CIRAN
     * @ araç gruplarının buyback ve trade back tanımlarını tanımlarını dropdown ya da tree ye doldurmak için sys_vehicles_trade tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args     
     * @return array
     * @throws \PDOException 
     */
    public function vehiclesBbDefDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
    ...
    /** 
     * @author Okan CIRAN
     * @  araç gruplarının en alt seviyesininde tanımlarını grid formatında döndürür !! ana tablo  sys_vehicles_trade 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillVehiclesTradeNamesGridx($params = array()) {
        try {
			...
			vgtm.model_description  gt_model_name, 
			a.description as trade_name, 
                        a.active,
                        COALESCE(NULLIF(sd16x.description, ''), sd16.description_eng) AS state_active, 
                        a.op_user_id,
                        u.username AS op_user_name,  
    ...
    /** 
     * @author Okan CIRAN
     * @  araç gruplarının en alt seviyesininde tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_vehicles_trade 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillVehiclesTradeNamesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
             $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicles_trade tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_vehicles_trade tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM sys_vehicles_trade_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
                    //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
