 ...
 * @since 30.07.2018                         
 */ 
class InfoCalendarEvents extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN    
     * @ info_calendar_events tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ info_calendar_events tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN info_calendar_events ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ info_calendar_events tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO info_calendar_events(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('info_calendar_events_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ info_calendar_events tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * info_calendar_events tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için info_calendar_events tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için info_calendar_events tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ info_calendar_events tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ takvim   tanımlarını grid formatında döndürür !! ana tablo  info_calendar_events 
     * @version v 1.0  20.08.2018
     * @param array | null $args 
     * @return array
     * @throws \PDOException  
     */  
    public function fillCalendarEventsGridx($params = array()) {
        try {
			...
			a.calendar_type_id,
			gt.name as calendar_types_name,   
			event_id ,
			a.name,
			a.description,
			a.event_start_date,
			a.event_end_date, 
                    ...
                    INNER JOIN info_users u ON u.id = a.event_user_id 
                    /*----*/    
                    inner join sys_calendar_types gt on gt.act_parent_id = a.calendar_type_id 
    ...
    /** 
     * @author Okan CIRAN
     * @  takvim tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo info_calendar_events
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCalendarEventsGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
    ...
    /**
     * @author Okan CIRAN
     * @ info_calendar_events tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ info_calendar_events tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
    ...
    public function insertAct($params = array()) {
        try {
            ...
            if ((isset($params['VehiclesEndgroupId']) && $params['VehiclesEndgroupId'] != "")) {
             ...
             $bodyBrand = null;
            if ((isset($params['BodyBrand']) && $params['BodyBrand'] != "")) {
                $bodyBrand = $params['BodyBrand'];
            }  
            $bodyOptions = null;
            if ((isset($params['BodyOptions']) && $params['BodyOptions'] != "")) {
                $bodyOptions = $params['BodyOptions'];
            } 
            $bodyDesc = null;
            if ((isset($params['BodyDesc']) && $params['BodyDesc'] != "")) {
                $bodyDesc = $params['BodyDesc'];
            }              
            $DemandDate= null;
            if ((isset($params['DemandDate']) && $params['DemandDate'] != "")) {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (  
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM info_calendar_events_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
          //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
