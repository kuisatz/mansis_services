 ...
 * @since 05.05.2016
 */
class SysVehicleTtogoType extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_vehicle_ttogo_type tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_ttogo_type tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  05.05.2016  
     * @param array $params
     * @return array
     * @throws \PDOException
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_vehicle_ttogo_type su ON (su.id = a.id OR su.language_parent_id = a.id) AND su.deleted =0 AND su.active =0 AND lx.id = su.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_ttogo_type tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  05.05.2016
     * @param type $params
     * @return array
     * @throws \PDOException
        ...
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    $sql = "
                INSERT INTO sys_vehicle_ttogo_type(                        
                        name, 
                        name_eng, 
                        link, 
                        logo, 
                ...
                VALUES (
                    ...
                    $insertID = $pdo->lastInsertId('sys_vehicle_ttogo_type_id_seq');
                    $errorInfo = $statement->errorInfo();
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_vehicle_ttogo_type tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    public function haveRecords($params = array()) {
        try {
            ...
            FROM sys_vehicle_ttogo_type  a                          
            WHERE 
                LOWER(TRIM(a.name)) = LOWER(TRIM('" . $params['name'] . "'))  
                  " . $addSql . " 
               AND a.deleted =0    
                               ";
            $statement = $pdo->prepare($sql);
    ...
    /**
     * @author Okan CIRAN
     * sys_vehicle_ttogo_type tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  05.05.2016
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicle_ttogo_type tablosundan kayıtları döndürür !!
     * @version v 1.0  05.05.2016
    ...
    /** 
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_vehicle_ttogo_type tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  05.05.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /** 
     * @author Okan CIRAN
     * @ sosyal medya bilgilerini dropdown ya da tree ye doldurmak için sys_vehicle_ttogo_type tablosundan kayıtları döndürür !!
     * @version v 1.0  05.05.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException 
     */
    public function vehicleTtogoTypeDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
            $languageId = NULL;
            $languageIdValue = 647;
            ...
            $statement = $pdo->prepare("        
              SELECT                    
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id =385 AND lx.deleted =0 AND lx.active =0   
                LEFT JOIN sys_vehicle_ttogo_type sd ON (sd.id =a.id OR sd.language_parent_id = a.id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE                    
                    a.deleted = 0 AND
