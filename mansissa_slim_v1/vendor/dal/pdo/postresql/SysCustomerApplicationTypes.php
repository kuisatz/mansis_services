 ...
 * @since 29.07.2018   
 */ 
class SysCustomerApplicationTypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    public function delete($params = array()) {
     try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                UPDATE sys_customer_application_types
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                FROM sys_customer_application_types a
                ...
                LEFT JOIN sys_customer_application_types ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_customer_application_types(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_customer_application_types_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_customer_application_types tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_application_types tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_application_types tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosundan parametre olarak  gelen id kaydın aktifliğini
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                SET active = (  SELECT   
                                CASE active
                                    ...
                                    ELSE 0
                                END activex
                                FROM sys_customer_application_types
                                WHERE id = " . intval($params['id']) . "
                ),
                op_user_id = " . intval($opUserIdValue) . "
                WHERE id = " . intval($params['id']);
    ...
    /** 
     * @author Okan CIRAN
     * @ araçların ne için kullanacağı ana grubu dropdown ya da tree ye doldurmak için sys_customer_application_types tablosundan kayıtları döndürür !!
    ...
    public function customerApplicationMainTypesDdList($params = array()) {
        try {
            ...
            $statement = $pdo->prepare("       
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_customer_application_types sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.show_it =0 AND lx.id = sd.language_id   
                WHERE   
                    a.parent_id = 0 AND
                    a.deleted = 0 AND
                    a.active =0 AND
    ...
    /** 
     * @author Okan CIRAN
     * @ araçların ne için kullanacağı alt grubu dropdown ya da tree ye doldurmak için sys_customer_application_types tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException 
    ...
    /** 
     * @author Okan CIRAN
     * @ Müşterinin araçların hangi uygulama da kullanacagı alan bilgileri tanımlarını grid formatında döndürür !! ana tablo  sys_customer_application_types 
    ...
    public function fillCustomerApplicationParentTypesGridx($params = array()) {
        try {
                    ...
                    if ($std['value'] != null) {
                        switch (trim($std['field'])) {
                            ...
                            case 'name_eng':
                             ...
                             case 'parent_name':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\' ';
                                $sorguStr.=" AND COALESCE(NULLIF(ay.name, ''), ay.name_eng)" . $sorguExpression . ' ';
            ...
            $customerApplicationTypeID =0 ;
            if (isset($params['CustomerApplicationTypeID']) && $params['CustomerApplicationTypeID'] != "") {
                $customerApplicationTypeID = $params['CustomerApplicationTypeID'];
                $addSql .="  a.act_parent_id  = " . intval($customerApplicationTypeID). "  AND  " ; 
            }  
			...
			a.parent_id,  
			COALESCE(NULLIF(ay.name, ''), ay.name_eng) AS parent_name,
			COALESCE(NULLIF(ax.description, ''), a.description_eng) AS description,
                      /*  a.name_eng, */                         
                        a.active,
                        COALESCE(NULLIF(sd16x.description, ''), sd16.description_eng) AS state_active,
                    ...
                    LEFT JOIN sys_customer_application_types ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.show_it = 0 AND ax.language_id = lx.id
		    LEFT JOIN sys_customer_application_types ay ON (ay.act_parent_id = a.parent_id ) AND ay.show_it = 0 AND ay.language_id = lx.id 
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @ Müşterinin araçların hangi uygulama da kullanacagı alan bilgileri  tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_customer_application_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_customer_application_types tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM sys_customer_application_types_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
                    //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_application_types tablosuna aktif olan diller için ,tek bir kaydın tabloda olmayan diğer dillerdeki kayıtlarını oluşturur   !!
     * @version v 1.0  26.08.2018
    ...
    public function insertLanguageTemplate($params = array()) {
        try {
			...
			case l.id when 385 then c.id else (SELECT last_value FROM sys_customer_application_types_id_seq) end as act_parent_id,  
                        c.op_user_id
                    FROM sys_customer_application_types c
                    LEFT JOIN sys_language l ON l.deleted =0 AND l.active =0 
                    WHERE c.id = " . intval($params['id']) . "  
                    ) AS xy   
                    WHERE xy.language_id NOT IN 
                        (SELECT DISTINCT language_id 
                        FROM sys_customer_application_types cx 
                        WHERE 
