 ...
 * @since 29.07.2018     
 */ 
class SysChannelTypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_channel_types tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_channel_types tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_channel_types ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_channel_types tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
        ...
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_channel_types_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_channel_types tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_channel_types tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_channel_types tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_channel_types tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_channel_types tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @  kitlerin getirilme tipi dropdown ya da tree ye doldurmak için sys_channel_types tablosundan kayıtları döndürür !!
    ...
    public function channelTypesDdList($params = array()) {
        try {
            ...
            $statement = $pdo->prepare("       
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_channel_types sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
     ...
     * @throws \PDOException  
     */  
    public function fillChannelTypesGridx($params = array()) {
        try {
            ...
            if (isset($params['BuybackTypeID']) && $params['BuybackTypeID'] != "") {
                ...
                $sql = "
                    ...
                    LEFT JOIN sys_channel_types ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @  kitlerin getirilme tip tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_channel_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillChannelTypesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_channel_types tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    VALUES (
                            ...
                            " . intval($opUserIdValue) . ",
                           (SELECT last_value FROM sys_channel_types_id_seq)
                                                 )   ";
                    $statement = $pdo->prepare($sql);
                    //   echo debugPDO($sql, $params);
                    $result = $statement->execute();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_channel_types tablosuna aktif olan diller için ,tek bir kaydın tabloda olmayan diğer dillerdeki kayıtlarını oluşturur   !!
     * @version v 1.0  26.08.2018
    ...
    public function insertLanguageTemplate($params = array()) {
        try {
			...
			case l.id when 385 then c.id else (SELECT last_value FROM sys_channel_types_id_seq) end as act_parent_id,  
                        c.op_user_id
                    FROM sys_channel_types c
                    LEFT JOIN sys_language l ON l.deleted =0 AND l.active =0 
                    WHERE c.id = " . intval($params['id']) . "  
                    ) AS xy   
                    WHERE xy.language_id NOT IN 
                        (SELECT DISTINCT language_id 
                        FROM sys_channel_types cx 
                        WHERE 
