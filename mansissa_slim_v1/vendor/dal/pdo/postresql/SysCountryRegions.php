 ...
 * @since 29.07.2018   
 */ 
class SysCountryRegions extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_country_regions tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_country_regions tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_country_regions ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_country_regions tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_country_regions tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_country_regions tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_country_regions tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_country_regions tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    public function  countryRegionsDdList($params = array()) {
        try {
            ...
            $statement = $pdo->prepare("       
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_country_regions sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
     ...
     * @throws \PDOException   
     */  
    public function fillCountryRegionsGridx($params = array()) {
        try {
            ...
            if (isset($params['CountryRegionID']) && $params['CountryRegionID'] != "") {
                ...
                $addSql ="  a.act_parent_id  = " . intval($countryRegionID). "  AND  " ; 
            }  
            $countryID =0 ;
            if (isset($params['CountryID']) && $params['CountryID'] != "") {
			...
			COALESCE(NULLIF(ax.name, ''), a.name_eng) AS name, 
                    ...
                    LEFT JOIN sys_country_regions ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
		    INNER JOIN sys_countrys c ON  c.act_parent_id = a.country_id AND c.show_it = 0 AND c.language_id = a.language_id 
		    LEFT JOIN sys_countrys cx ON (cx.act_parent_id = c.act_parent_id OR cx.language_parent_id = c.act_parent_id) AND cx.show_it = 0 AND cx.language_id = lx.id
    ...
    /** 
     * @author Okan CIRAN
     * @ ülke bölgeleri tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_country_regions 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCountryRegionsGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_country_regions tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
