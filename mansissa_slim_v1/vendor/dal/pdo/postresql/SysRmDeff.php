 ...
 * @since 30.07.2018   
 */ 
class SysRmDeff extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN  
     * @ sys_rm_deff tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_rm_deff tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_rm_deff ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_rm_deff tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_rm_deff tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_rm_deff tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_rm_deff tablosundan parametre olarak  gelen id kaydın aktifliğini
     ...
     * @throws \PDOException 
     */
    public function  rmDeffDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
            $languageIdValue = 385;
            if (isset($params['language_code']) && $params['language_code'] != "") { 
            ...
            $statement = $pdo->prepare("       
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_rm_deff sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
     ...
     * @throws \PDOException  
     */  
    public function fillRmDeffsGridx($params = array()) {
        try {
            ...
            if (isset($params['RmTypeID']) && $params['RmTypeID'] != "") {
                ...
                $addSql ="  a.rm_type_id = " . intval($rmTypeID). "  AND  " ; 
            }  
                $sql = "
                     SELECT  
                        a.id, 
		    ...
		    INNER JOIN sys_mileages drd ON drd.act_parent_id = a.big_mileage_id AND drd.show_it = 0 AND drd.language_id= l.id
		    ...
		    INNER JOIN sys_mileages erd ON erd.act_parent_id = a.small_mileage_id AND erd.show_it = 0 AND erd.language_id= l.id
		   ...
		   INNER JOIN sys_rm_types grd ON grd.act_parent_id = a.rm_type_id AND grd.show_it = 0 AND grd.language_id= l.id
    ...
    /** 
     * @author Okan CIRAN
     * @ R&M tanım bilgilerini grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_rm_deff 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_rm_deff tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
