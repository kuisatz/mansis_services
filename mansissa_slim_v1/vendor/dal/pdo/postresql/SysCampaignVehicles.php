 ...
 * @since 29.07.2018  
 */ 
class SysCampaignVehicles extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_campaign_vehicles tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_campaign_vehicles tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_campaign_vehicles ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_campaign_vehicles tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_campaign_vehicles(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_campaign_vehicles_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_campaign_vehicles tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_campaign_vehicles tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_campaign_vehicles tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_campaign_vehicles tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /** 
     * @author Okan CIRAN
     * @ kampanya grid formatında döndürür !! ana tablo  sys_campaign_vehicles 
    ...
    public function fillCampaignVehiclesGridx($params = array()) {
        try {
            ...
            if (isset($params['VehicleGroupsID']) && $params['VehicleGroupsID'] != "") {
                ...
                $addSql .="  vet.vehicle_groups_id  = " . intval($VehicleGroupsID). "  AND  " ; 
            }  
            $CampaignID =0 ;
            if (isset($params['CampaignID']) && $params['CampaignID'] != "") {
                $CampaignID = $params['CampaignID'];
                $addSql .="  a.campaign_id  = " . intval($CampaignID). "  AND  " ; 
            }    
                ...
                $sql = "
                   SELECT 
                        ...
                        a.campaign_id,
                        COALESCE(NULLIF(bx.name, ''), b.name_eng) AS campaign_name,
                        vet.vehicle_groups_id, 
                        vg.name as vehichle_group_name, 
			a.stock_vehicle_type_id, 
                        gt.model_description , 
			 ...
			 a.stock_vehicle_id,
			  ...
			  a.mfs_support ,
                    ...
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/
		    INNER JOIN sys_campaigns b ON b.act_parent_id = a.campaign_id AND b.show_it = 0 AND b.language_id= l.id
                    LEFT JOIN sys_campaigns bx ON (bx.act_parent_id = b.act_parent_id OR bx.language_parent_id= b.act_parent_id) AND bx.show_it = 0 AND bx.language_id =lx.id  

		   
	           
		 LEFT JOIN sys_vehicle_gt_models gt ON gt.act_parent_id = a.stock_vehicle_type_id AND b.show_it = 0  
		 LEFT JOIN sys_vehicle_group_types vet ON vet.act_parent_id = gt.vehicle_group_types_id AND vet.show_it = 0  
		 LEFT JOIN sys_vehicle_groups vg ON vg.act_parent_id = vet.vehicle_groups_id AND vg.show_it = 0 
    ...
    /** 
     * @author Okan CIRAN
     * @ kampanya  grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_campaign_vehicles 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
