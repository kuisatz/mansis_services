 ...
 * @since 29.07.2018 
 */ 
class SysAccDeff extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_acc_deff tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_acc_deff tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_acc_deff ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_acc_deff tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_acc_deff tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_acc_deff tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_acc_deff tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ salesman in göreceği aksesuar tanımları dropdown ya da tree ye doldurmak için sys_acc_deff tablosundan kayıtları döndürür !!
    ...
    public function  accDeffSalesmanDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                SELECT                    
                    ...
                    COALESCE(NULLIF(sd.name_sm, ''), a.name_sm_eng) AS name,  
                    a.name_sm_eng AS name_eng,
                     0 as parent_id,
                    a.active,
                    0 AS state_type   
                FROM sys_acc_deff a    
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_acc_deff sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
     ...
     /** 
     * @author Okan CIRAN
     * @ backoffice in göreceği aksesuar tanımları dropdown ya da tree ye doldurmak için sys_acc_deff tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException 
    ...
    public function  accDeffBackOfficeDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                SELECT                    
                    a.act_parent_id AS id, 	
                    COALESCE(NULLIF(sd.name_bo, ''), a.name_bo_eng) AS name,  
                    a.name_bo_eng AS name_eng,
                     0 as parent_id,
                    a.active,
                    0 AS state_type   
                FROM sys_acc_deff a    
    ...
    /** 
     * @author Okan CIRAN
     * @ salesman ve backoffice elemanlarının göreceği  aksesuar tanımları grid formatında döndürür !! ana tablo  sys_acc_deff 
                    ...
                    if ($std['value'] != null) {
                        switch (trim($std['field'])) {
                            case 'name_sa':
                                ...
                                $sorguStr.=" AND COALESCE(NULLIF(ax.name_sm, ''), a.name_sm_eng)" . $sorguExpression . ' ';
                              
                                break;
                              case 'name_bo':
                                $sorguExpression = ' ILIKE \'%' . $std['value'] . '%\' ';
                                $sorguStr.=" AND COALESCE(NULLIF(ax.name_bo, ''), a.name_bo_eng) " . $sorguExpression . ' ';
    ...
    /** 
     * @author Okan CIRAN 
     * @ salesman ve backoffice elemanlarının göreceği aksesuar tanımları  grid formatında gösterilirken kaç kayıt olduğunu döndürür !!  ana tablo  sys_acc_deff 
     * @version v 1.0  15.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
