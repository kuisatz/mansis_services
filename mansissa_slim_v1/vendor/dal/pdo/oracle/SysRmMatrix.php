 ...
 * @since 30.07.2018   
 */ 
class SysRmMatrix extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN  
     * @ sys_rm_matrix tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_rm_matrix tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_rm_matrix ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_rm_matrix tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_rm_matrix tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_rm_matrix tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_rm_matrix tablosundan parametre olarak  gelen id kaydın aktifliğini
     ...
     * @throws \PDOException  
     */  
    public function fillRmMatrixGridx($params = array()) {
        try {
            ...
            if (isset($params['RmTypeID']) && $params['RmTypeID'] != "") {
                $rmTypeID = $params['RmTypeID'];
                $addSql ="  crd.rm_type_id = " . intval($rmTypeID). "  AND  " ; 
            }  
                $sql = "
                      SELECT  
                        a.id, 
			...
			crd.small_mileage_id,
			COALESCE(NULLIF(erdx.name, ''), erd.name_eng) AS small_mileage,
			a.coefficient,
			a.price, 
			cast( a.price *  erd.mileages1 * frd.mvalue /100 as numeric(15,4)) price1,
			cast( a.coefficient *  erd.mileages1 * frd.mvalue /100 as numeric(15,4)) price2 , 
                    ...
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
		    INNER JOIN sys_vehicle_gt_models vgtm ON vgtm.act_parent_id = a.model_id AND vgtm.show_it = 0  
		    
		    INNER JOIN sys_rm_deff crd ON crd.act_parent_id = a.rm_deff_id AND crd.show_it = 0  
 
		    INNER JOIN sys_mileages drd ON drd.act_parent_id = crd.big_mileage_id AND drd.show_it = 0 AND drd.language_id= l.id
		    LEFT JOIN sys_mileages drdx ON (drdx.act_parent_id = drd.act_parent_id OR drdx.language_parent_id= drd.act_parent_id) AND drdx.show_it= 0 AND drdx.language_id =lx.id  

		    INNER JOIN sys_mileages erd ON erd.act_parent_id = crd.small_mileage_id AND erd.show_it = 0 AND erd.language_id= l.id
		    ...
		    INNER JOIN sys_months frd ON frd.act_parent_id = crd.month_id AND frd.show_it = 0 AND frd.language_id= l.id
		   ...
		   INNER JOIN sys_rm_types grd ON grd.act_parent_id = crd.rm_type_id AND grd.show_it = 0 AND grd.language_id= l.id
		    LEFT JOIN sys_rm_types grdx ON (grdx.act_parent_id = grd.act_parent_id OR grdx.language_parent_id= grd.act_parent_id) AND grdx.show_it= 0 AND grdx.language_id =lx.id  
                  ...
                  /*  order by  
                    ...
                    COALESCE(NULLIF(drdx.name, ''), drd.name_eng),
                    COALESCE(NULLIF(erdx.name, ''), erd.name_eng) */ 
    ...
    /** 
     * @author Okan CIRAN
     * @ R&M matrix tanım bilgilerini grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_rm_matrix 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillRmMatrixGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_rm_matrix tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
