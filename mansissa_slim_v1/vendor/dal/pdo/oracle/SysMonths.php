 ...
 * @since 30.07.2018    
 */ 
class SysMonths extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_months tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    /**
     * @author Okan CIRAN
     * @ sys_months tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_months ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_months tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
        ...
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_months_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_months tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_months tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  30.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_months tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_months tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_months tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ ay tipleri ana grupları dropdown ya da tree ye doldurmak için sys_months tablosundan kayıtları döndürür !!
    ...
    public function  monthsMainGroupDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_months sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.parent_id =0 AND
    ...
    /** 
     * @author Okan CIRAN
     * @ ay tipleri alt grupları dropdown ya da tree ye doldurmak için sys_months tablosundan kayıtları döndürür !!
     * @version v 1.0  11.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException 
    ...
    /** 
     * @author Okan CIRAN
     * @ ay tipleri tanımlarını grid formatında döndürür !! ana tablo  sys_months 
    ...
    public function fillMonthsPGridx($params = array()) {
        try {
			...
			a.parent_id,   
                    ...
                    LEFT JOIN sys_months ax ON (ax.act_parent_id =a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id   
               
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
		    INNER JOIN sys_months drd ON drd.act_parent_id = a.parent_id AND drd.active =0 AND drd.deleted = 0 AND drd.language_id= l.id and drd.parent_id =0 
		    LEFT JOIN sys_months drdx ON (drdx.act_parent_id = drd.act_parent_id OR drdx.language_parent_id= drd.act_parent_id) AND drdx.active =0 AND drdx.deleted = 0 AND drdx.language_id =lx.id  
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @ ay tipleri tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo sys_months
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
    ...
    /**
     * @author Okan CIRAN
     * @ sys_months tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_months tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
