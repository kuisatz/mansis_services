 ...
 * @since 30.07.2018      
 */ 
class SysSalesProvisionTypes extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN    
     * @ sys_sales_provision_types tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    public function delete($params = array()) {
     try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                UPDATE sys_sales_provision_types
    ...
    /**
     * @author Okan CIRAN
     * @ sys_sales_provision_types tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  30.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_sales_provision_types ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_sales_provision_types tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_sales_provision_types tablosundan kayıtları döndürür !!
     * @version v 1.0  30.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_sales_provision_types tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  30.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_sales_provision_types tablosundan parametre olarak  gelen id kaydın aktifliğini
    ...
    /** 
     * @author Okan CIRAN
     * @ sales provision types dropdown ya da tree ye doldurmak için sys_sales_provision_types tablosundan kayıtları döndürür !!
    ...
    public function  salesProvisionTypesDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                SELECT                    
                    ...
                    COALESCE(NULLIF(sd.abbrevation, ''), a.abbrevation_eng) AS name,  
                    a.abbrevation_eng AS name_eng,
                     0 as parent_id,
                    a.active,
                    0 AS state_type   
                FROM sys_sales_provision_types a    
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_sales_provision_types sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
    ...
    public function fillSalesProvisionTypesGridx($params = array()) {
        try {
            ...
            if (isset($params['CbuCkdTypeID']) && $params['CbuCkdTypeID'] != "") {
                $cbuCkdTypeID = $params['CbuCkdTypeID'];
                $addSql ="  a.cbu_ckd_type_id  = " . intval($cbuCkdTypeID). "  AND  " ; 
            }  
    ...
    /** 
     * @author Okan CIRAN
     * @ sales provision types tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_sales_provision_types 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_sales_provision_types tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
