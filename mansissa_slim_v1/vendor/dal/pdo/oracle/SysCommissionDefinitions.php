 ...
 * @since 29.07.2018  
 */ 
class SysCommissionDefinitions extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    public function delete($params = array()) {
     try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                UPDATE sys_commission_definitions
    ...
    /**
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                FROM sys_commission_definitions a
                ...
                LEFT JOIN sys_commission_definitions ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_commission_definitions(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_commission_definitions_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_commission_definitions tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_commission_definitions tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_commission_definitions tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosundan parametre olarak  gelen id kaydın aktifliğini
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                SET active = (  SELECT   
                                CASE active
                                    ...
                                    ELSE 0
                                END activex
                                FROM sys_commission_definitions
                                WHERE id = " . intval($params['id']) . "
                ),
                op_user_id = " . intval($opUserIdValue) . "
                WHERE id = " . intval($params['id']);
    ...
    /** 
     * @author Okan CIRAN
     * @ komisyon tanımları dropdown ya da tree ye doldurmak için sys_commission_definitions tablosundan kayıtları döndürür !!
    ...
    public function  commissionDefinitionsDdList($params = array()) {
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_commission_definitions sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
    ...
    /** 
     * @author Okan CIRAN
     * @ komisyon tanımları tanımlarını grid formatında döndürür !! ana tablo  sys_commission_definitions 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCommissionDefinitionsGridx($params = array()) {
        try {
            ...
            $commissionDefinitionID =0 ;
            if (isset($params['CommissionDefinitionID']) && $params['CommissionDefinitionID'] != "") {
                $commissionDefinitionID = $params['CommissionDefinitionID'];
                $addSql ="  a.act_parent_id  = " . intval($commissionDefinitionID). "  AND  " ; 
            }  
                ...
                $sql = "
                    ...
                    LEFT JOIN sys_commission_definitions ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN
     * @ komisyon tanımları tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_commission_definitions 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCommissionDefinitionsGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
            ...
            if (isset($params['CommissionDefinitionID']) && $params['CommissionDefinitionID'] != "") {
                ...
                $sql = "
                   SELECT COUNT(asdx.id) count FROM ( 
                        ...
                        INNER JOIN sys_language l ON l.id = a.language_id AND l.show_it =0
                        LEFT JOIN sys_language lx ON lx.id =" . intval($languageIdValue) . "    AND lx.show_it =0  
                        LEFT JOIN sys_calendar_types ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                        INNER JOIN info_users u ON u.id = a.op_user_id 
                        /*----*/   
                       /* INNER JOIN sys_specific_definitions sd15 ON sd15.main_group = 15 AND sd15.first_group= a.deleted AND sd15.deleted =0 AND sd15.active =0 AND sd15.language_parent_id =0 */
                        INNER JOIN sys_specific_definitions sd16 ON sd16.main_group = 16 AND sd16.first_group= a.active AND sd16.deleted = 0 AND sd16.active = 0 AND sd16.language_id =l.id
     ...
     /**
     * @author Okan CIRAN
     * @ sys_commission_definitions tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_commission_definitions tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
