 ...
 * @
 * @author Okan CIRAN
 * @since 29.07.2018   SysCustomerTurnoverRates
 */ 
class SysCustomerTurnoverRates extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosundan parametre olarak  gelen id kaydını siler. !!
    ...
    public function delete($params = array()) {
     try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                UPDATE sys_customer_turnover_rates
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  29.07.2018  
     * @param array $params
     * @return array
     * @throws \PDOException 
    ...
    public function getAll($params = array()) {
        try {
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                FROM sys_customer_turnover_rates a
                ...
                LEFT JOIN sys_customer_turnover_rates ax ON (ax.id = a.id OR ax.language_parent_id = a.id) AND ax.deleted =0 AND ax.active =0 AND lx.id = ax.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    public function insert($params = array()) {
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                $sql = "
                INSERT INTO sys_customer_turnover_rates(
                        name, 
                        name_eng, 
                        description, 
                        description_eng, 
                ...
                //   echo debugPDO($sql, $params);
                    ...
                    $insertID = $pdo->lastInsertId('sys_customer_turnover_rates_id_seq');
                    $errorInfo = $statement->errorInfo();  
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosunda property_name daha önce kaydedilmiş mi ?  
    ...
    /**
     * @author Okan CIRAN
     * sys_customer_turnover_rates tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  29.07.2018
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_turnover_rates tablosundan kayıtları döndürür !!
     * @version v 1.0  29.07.2018
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_turnover_rates tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  29.07.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /*
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosundan parametre olarak  gelen id kaydın aktifliğini
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                SET active = (  SELECT   
                                CASE active
                                    ...
                                    ELSE 0
                                END activex
                                FROM sys_customer_turnover_rates
                                WHERE id = " . intval($params['id']) . "
                ),
                op_user_id = " . intval($opUserIdValue) . "
                WHERE id = " . intval($params['id']);
     ...
     * @throws \PDOException 
     */
    public function  customerTurnoverRatesDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('oracleConnectFactory');         
            $languageIdValue = 385;
            if (isset($params['language_code']) && $params['language_code'] != "") { 
		...
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id = " . intval($languageIdValue). "  AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_customer_turnover_rates sd ON (sd.act_parent_id =a.act_parent_id OR sd.language_parent_id = a.act_parent_id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE   
                    a.deleted = 0 AND
                    a.active =0 AND
                    a.language_parent_id =0 
     ...
     * @throws \PDOException   
     */  
    public function fillCustomerTurnoverRatesGridx($params = array()) {
        try {
            ...
            $customerTurnoverRateID =0 ;
            if (isset($params['CustomerTurnoverRateID']) && $params['CustomerTurnoverRateID'] != "") {
                $customerTurnoverRateID = $params['CustomerTurnoverRateID'];
                $addSql ="  a.act_parent_id  = " . intval($customerTurnoverRateID). "  AND  " ; 
            }  
                ...
                $sql = "
                    ...
                    LEFT JOIN sys_customer_turnover_rates ax ON (ax.act_parent_id = a.act_parent_id OR ax.language_parent_id = a.act_parent_id) AND ax.deleted = 0 AND ax.active = 0 AND ax.language_id = lx.id
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
    ...
    /** 
     * @author Okan CIRAN  
     * @  ciro aralığı  tanımlarını grid formatında gösterilirken kaç kayıt olduğunu döndürür !! ana tablo  sys_customer_turnover_rates 
     * @version v 1.0  20.08.2018
     * @param array | null $args
     * @return array
     * @throws \PDOException  
     */  
    public function fillCustomerTurnoverRatesGridxRtl($params = array()) {
        try {             
            $sorguStr = null;    
            $addSql = null;
            if (isset($params['filterRules'])) {
     ...
     /**
     * @author Okan CIRAN
     * @ sys_customer_turnover_rates tablosundan parametre olarak  gelen id kaydını active ve show_it alanlarını 1 yapar. !!
    ...
    /**
     * @author Okan CIRAN     
     * @ sys_customer_turnover_rates tablosundan parametre olarak  gelen id kaydın active veshow_it  alanını 1 yapar ve 
