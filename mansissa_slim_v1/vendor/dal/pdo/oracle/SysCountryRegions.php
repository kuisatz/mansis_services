    ...
    public function fillCountryRegionsGridx($params = array()) {
        try {
			...
			a.act_parent_id,    
                    ...
                    INNER JOIN info_users u ON u.id = a.op_user_id 
                    /*----*/   
		    INNER JOIN sys_countrys c ON  c.act_parent_id = a.country_id AND c.deleted = 0 AND c.active = 0 AND c.language_id = a.language_id 
		    LEFT JOIN sys_countrys cx ON (cx.act_parent_id = c.act_parent_id OR cx.language_parent_id = c.act_parent_id) AND cx.deleted = 0 AND cx.active = 0 AND cx.language_id = lx.id
