 ...
 * @since 15-08-2018
 */
$app->get("/pkFillBBSpecialGridx_sysaccbodymatrix/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysAccBodyMatrixBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillBBSpecialGridx_sysaccbodymatrix" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
