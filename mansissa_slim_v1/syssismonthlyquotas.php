 ...
 * @since 11.08.2018
 */
$app->get("/pkSisQuotasDdList_syssismonthlyquotas/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysSisMonthlyQuotasBLL');
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkSisQuotasDdList_syssismonthlyquotas" end point, X-Public variable not found');
 ...
 * @since 15-08-2018
 */
$app->get("/pkFillSisMonthlyQuotasGridx_syssismonthlyquotas/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysSisMonthlyQuotasBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillAccBodyDeffGridx_syssismonthlyquotas" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillSisMonthlyQuotasGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
    ...
    $resTotalRowCount = $BLL->fillSisMonthlyQuotasGridxRtl(array(
        'language_code' => $vLanguageCode, 
        'LanguageID' => $lid,
 ...
 * @since 15-08-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_syssismonthlyquotas/", function () use ($app ) {
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkInsertAct_syssismonthlyquotas/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysSisMonthlyQuotasBLL');  
    $headerParams = $app->request()->headers();
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkUpdateAct_syssismonthlyquotas/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysSisMonthlyQuotasBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdateAct_syssismonthlyquotas" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */
$app->get("/pkDeletedAct_syssismonthlyquotas/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysSisMonthlyQuotasBLL');   
    $headerParams = $app->request()->headers();
