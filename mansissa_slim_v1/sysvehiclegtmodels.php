 ...
 * @since 15-08-2018
 */
$app->get("/fillAccBodyDeffGridx_sysvehiclegtmodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysVehicleGtModelsBLL');
   /* $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "fillAccBodyDeffGridx_sysvehiclegtmodels" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
*/
    $vLanguageCode = 'en';
    if (isset($_GET['language_code'])) {
