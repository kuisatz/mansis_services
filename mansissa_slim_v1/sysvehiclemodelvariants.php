 ...
 * @since 15-08-2018
 */
$app->get("/pkFillVehicleModelVariantsGridx_sysvehiclemodelvariants/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysVehicleModelVariantsBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillVehicleModelVariantsGridx_sysvehiclemodelvariants" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillVehicleModelVariantsGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
    ...
    $resTotalRowCount = $BLL->fillVehicleModelVariantsGridxRtl(array(
        'language_code' => $vLanguageCode, 
        'LanguageID' => $lid,
