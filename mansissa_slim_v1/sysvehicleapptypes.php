 ...
 * @since 11.08.2018
 */
$app->get("/pkVehicleAppTypesDdList_sysvehicleapptypes/", function () use ($app ) {
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkVehicleAppTypesDdList_sysvehicleapptypes" end point, X-Public variable not found');
    ...
    $resCombobox = $BLL->vehicleAppTypesDdList(array(                                   
 ...
 * @since 15-08-2018
 */
$app->get("/pkFillvehicleAppTypesGridx_sysvehicleapptypes/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysVehicleAppTypesBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillvehicleAppTypesGridx_sysvehicleapptypes" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillvehicleAppTypesGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
