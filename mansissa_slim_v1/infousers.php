 ...
 * @since 05.08.2018
 */
$app->get("/pkSalesmanDdList_infoUsers/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('infoUsersBLL');
    ...
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkSalesmanDdList_infoUsers" end point, X-Public variable not found');
   // $pk = $headerParams['X-Public'];
