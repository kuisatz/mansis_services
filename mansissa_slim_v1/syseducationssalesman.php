 ...
 * @since 11.08.2018
 */
$app->get("/pkEducationDefinitionsDdList_syseducationssalesman/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysEducationsSalesmanBLL');
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkEducationDefinitionsDdList_syseducationssalesman" end point, X-Public variable not found');
 ...
 * @since 15-08-2018
 */
$app->get("/pkFillEducationsSalesmanGridx_syseducationssalesman/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysEducationsSalesmanBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillEducationsSalesmanGridx_syseducationssalesman" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillEducationsSalesmanGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
    ...
    $resTotalRowCount = $BLL->fillEducationsSalesmanGridxRtl(array(
        'language_code' => $vLanguageCode, 
        'LanguageID' => $lid,
        ...
        foreach ($resDataGrid as $menu) {
            $menus[] = array(
               "id" => $menu["id"],
                ...
                "user_id" => intval($menu["user_id"]),  
                "name_surname" => html_entity_decode($menu["name_surname"]), 
                ...
                "edu_start_date_date" => intval($menu["edu_start_date_date"]),  
 ...
 * @since 15-08-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_syseducationssalesman/", function () use ($app ) {
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkInsertAct_syseducationssalesman/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysEducationsSalesmanBLL');  
    $headerParams = $app->request()->headers();
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkUpdateAct_syseducationssalesman/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysEducationsSalesmanBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdateAct_syseducationssalesman" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */
$app->get("/pkDeletedAct_syseducationssalesman/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysEducationsSalesmanBLL');   
    $headerParams = $app->request()->headers();
