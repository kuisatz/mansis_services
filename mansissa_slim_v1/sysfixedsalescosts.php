 ...
 * @since 11.08.2018
 */
$app->get("/pkFixedSalesCostsDdList_sysfixedsalescosts/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysFixedSalesCostsBLL');
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkFixedSalesCostsDdList_sysfixedsalescosts" end point, X-Public variable not found');
    ...
    $resCombobox = $BLL->fixedSalesCostsDdList(array(                                   
 ...
 * @since 15-08-2018
 */
$app->get("/pkFixedSalesCostsGridx_sysfixedsalescosts/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysFixedSalesCostsBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFixedSalesCostsGridx_sysfixedsalescosts" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fixedSalesCostsGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
    ...
    $resTotalRowCount = $BLL->fixedSalesCostsGridxRtl(array(
        'language_code' => $vLanguageCode, 
        'LanguageID' => $lid,
        'AccBodyTypeID' => $vAccBodyTypeID,
        'filterRules' => $filterRules,
 ...
 * @since 15-08-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_sysfixedsalescosts/", function () use ($app ) {
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkInsertAct_sysfixedsalescosts/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysFixedSalesCostsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkInsertAct_sysfixedsalescosts" end point, X-Public variable not found');    
     $pk =  $headerParams['X-Public'];
    ...
    $CurrencyTypeId = NULL;
    if (isset($_GET['currency_type_id'])) {
         $stripper->offsetSet('currency_type_id',$stripChainerFactory->get(stripChainers::FILTER_ONLY_NUMBER_ALLOWED,
                                                $app,
                                                $_GET['currency_type_id']));
    } 
    $vvalue = NULL;
    if (isset($_GET['vvalue'])) {
         $stripper->offsetSet('vvalue',$stripChainerFactory->get(stripChainers::FILTER_PARANOID_LEVEL2,
                                                $app,
                                                $_GET['vvalue']));
    }  
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkUpdateAct_sysfixedsalescosts/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysFixedSalesCostsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdateAct_sysfixedsalescosts" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */
$app->get("/pkDeletedAct_sysfixedsalescosts/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysFixedSalesCostsBLL');   
    $headerParams = $app->request()->headers();
