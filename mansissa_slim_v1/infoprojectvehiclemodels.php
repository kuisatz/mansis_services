 ...
 * @since 05.08.2018
 */
$app->get("/pkProjectVehicleModelsDdList_infoprojectvehiclemodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('infoProjectVehicleModelsBLL');
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkProjectVehicleModelsDdList_infoprojectvehiclemodels" end point, X-Public variable not found');
 ...
 * @since 15-08-2018
 */
$app->get("/pkFillProjectVehicleModelsGridx_infoprojectvehiclemodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('infoProjectVehicleModelsBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillProjectVehicleModelsGridx_infoprojectvehiclemodels" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillProjectVehicleModelsGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
 ...
 * @since 15-08-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_infoprojectvehiclemodels/", function () use ($app ) {
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkInsertAct_infoprojectvehiclemodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('infoProjectVehicleModelsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkInsertAct_infoprojectvehiclemodels" end point, X-Public variable not found');    
     $pk =  $headerParams['X-Public'];
    ...
    if (isset($_GET['delivery_date'])) {
         $stripper->offsetSet('delivery_date',$stripChainerFactory->get(stripChainers::FILTER_PARANOID_LEVEL2,
                                                $app,
                                                $_GET['delivery_date']));
    }   
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkUpdateAct_infoprojectvehiclemodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('infoProjectVehicleModelsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdateAct_infoprojectvehiclemodels" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */
$app->get("/pkDeletedAct_infoprojectvehiclemodels/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('infoProjectVehicleModelsBLL');   
    $headerParams = $app->request()->headers();
