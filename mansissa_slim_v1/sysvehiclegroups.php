 ...
 * @since 11.08.2018
 */
$app->get("/pkVehicleGroupsDdList_sysvehiclegroups/", function () use ($app ) {
    ...
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkVehicleGroupsDdList_sysvehiclegroups" end point, X-Public variable not found');
...
$app->get("/pkFillVehicleGroupsGridx_sysvehiclegroups/", function () use ($app ) {
    ...
    $resDataGrid = $BLL->fillVehicleGroupsGridx(array(
        'language_code' => $vLanguageCode,
        'LanguageID' => $lid,
        'page' => $vPage,
        'rows' => $vRows,
    ...
    $resTotalRowCount = $BLL->fillVehicleGroupsGridxRtl(array(
        'language_code' => $vLanguageCode, 
        'LanguageID' => $lid,
 ...
 * @since 15-08-2018
 */
$app->get("/fillAccBodyDeffGridx_sysvehiclegroups/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('sysVehicleGroupsBLL');
   /* $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "fillAccBodyDeffGridx_sysvehiclegroups" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
*/
    $vLanguageCode = 'en';
    if (isset($_GET['language_code'])) {
 ...
 * @since 15-08-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_sysvehiclegroups/", function () use ($app ) {
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkInsertAct_sysvehiclegroups/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysVehicleGroupsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkInsertAct_sysvehiclegroups" end point, X-Public variable not found');    
     $pk =  $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */ 
$app->get("/pkUpdateAct_sysvehiclegroups/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysVehicleGroupsBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdateAct_sysvehiclegroups" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
 ...
 * @since 15-08-2018
 */
$app->get("/pkDeletedAct_sysvehiclegroups/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysVehicleGroupsBLL');   
    $headerParams = $app->request()->headers();
